...
def stream():
    global stream_model, first_run_flag, manager_dict
    ...
    manager_dict['stream_model'] = stream_model
    return render_template('stream.html')

def gen():
    """Video streaming generator function."""
    global stream_model, font, manager_dict

    while True:
        frame = manager_dict['frame']
...
if __name__ == '__main__':
    frame = np.ones((512, 1024), dtype=np.int8)*254
    cv2.putText(frame, "init Camera is not available", (7,70), font, 2, (0, 255, 0), 2)
    
    manager_dict['frame'] = frame
    manager_dict['stream_model'] = 'yolov7-tiny_256x320.onnx'
    
    p = Process(target=camera_process, args=(manager_dict,))
    p.start()
